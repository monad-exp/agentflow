...
| `bugfinder/v3/run.py` | You want the Smithers bugfinder-v3 security workflow without a database: budgeted file and goal hunts, threat-model synthesis, three-gate triage and per-finding reports. | `output_json_schema` criteria, python collectors as the only fan-out sources, budget profiles from `config.yaml`, provider pools, vendored bounty policy, known-findings JSONL corpus. |
